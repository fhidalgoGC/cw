...
import { WashType, getEstimatedTime } from "@/lib/storage";
...
function formatDuration(minutes: number): string {
  ...
  if (remainder === 30) return hours === 1 ? "1 hora y media" : `${hours} horas y media`;
...
interface SchedulePickerProps {
  washType: WashType;
  addOns: string[];
  onConfirm: (date: Date, time: string, reservationExpiry: number) => void;
  showEstimatedTime?: boolean;
}

export function SchedulePicker({ washType, addOns, onConfirm, showEstimatedTime = true }: SchedulePickerProps) {
  ...
  const handleConfirm = () => {
    if (selectedDate && selectedTime && expiryRef.current) {
      ...
      onConfirm(selectedDate, selectedTime, expiryRef.current);
    }
  };
    ...
    <View style={styles.container}>
      {isReserved ? (
        <Animated.View
          ...
          style={[styles.timerBar, { backgroundColor: timerColor + "15" }]}
        >
          ...
          <View style={[styles.timerProgressBg, { backgroundColor: timerColor + "20" }]}>
            <View
              style={[
                ...
                { backgroundColor: timerColor, width: `${timerProgress * 100}%` },
              ]}
            />
          </View>
        </Animated.View>
      ...
      <View style={[styles.footer, { backgroundColor: theme.backgroundDefault }]}>
        ...
        >
          <ThemedText
            type="body"
            style={{
              color: selectedDate && selectedTime ? "#FFFFFF" : theme.textSecondary,
              fontWeight: "700",
              textAlign: "center",
            }}
          >
